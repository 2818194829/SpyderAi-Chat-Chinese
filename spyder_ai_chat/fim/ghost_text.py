...
class _GhostEventFilter(QObject):
    ...
    def __init__(self, manager, editor):
        # Parent to None so we can install on QApplication without
        # the editor's lifetime constraining us.
        super().__init__(None)
    ...
    def eventFilter(self, obj, event):
        ...
        Installed at QApplication level so Escape is caught before
        QScintilla's internal handler consumes it.
        Only acts when the event target is our editor or its viewport.
        """
        etype = event.type()

        # ── Escape via ShortcutOverride ────────────────────────────────
        # Spyder registers a QShortcut for Escape, so Qt delivers Escape
        # as ShortcutOverride (type 51) to check if any widget wants to
        # claim it. If nothing accepts, the shortcut fires and the actual
        # KeyPress is never sent. We must intercept ShortcutOverride here:
        # call clear(), accept the event (blocks the Spyder shortcut), and
        # return True (consume it). Qt may then send a KeyPress but by that
        # point has_suggestion() is False so it passes through harmlessly.
        if (etype == QEvent.ShortcutOverride
                and event.key() == Qt.Key_Escape
                and self._manager.has_suggestion()):
            self._manager.clear()
            event.accept()
            return True
        ...
        if key == Qt.Key_Escape:
            # Fallback: Escape arrived as a plain KeyPress (no Spyder shortcut
            # active in this context). Clear and let it propagate.
            self._manager.clear()
            return False

        # Only intercept Tab / typed-prefix events directed at our editor or
        # its viewport — ignore key events meant for unrelated widgets.
        editor = self._editor
        viewport = getattr(editor, "viewport", None)
        if callable(viewport):
            viewport = viewport()
        if obj is not editor and obj is not viewport:
            return False
...
class GhostTextManager:
    ...
    def __init__(self, editor):
        ...
        # Install on QApplication so we get events BEFORE QScintilla's
        # internal key handler. QScintilla consumes Escape internally
        # (for its own autocomplete popup) before editor-level event
        # filters ever see it. Application-level filters run first.
        from qtpy.QtWidgets import QApplication
        app = QApplication.instance()
        if app is not None:
            app.installEventFilter(self._event_filter)
            self._event_targets = [app]
        else:
    ...
    def show_suggestion(self, text, target=None):
        ...
        # Reinstall event filter on QApplication so it runs BEFORE any
        # filter installed more recently (e.g. an LSP popup filter that
        # might otherwise consume Escape before we see it).
        # Qt event filters are LIFO: last installed runs first.
        from qtpy.QtWidgets import QApplication
        app = QApplication.instance()
        if app is not None:
    ...
    def clear(self):
        ...
        Primary path: doc.undo() to cleanly reverse the insertion (no new
        undo step). Fallback: direct cursor deletion if undo didn't work
        (can happen if something else pushed onto the undo stack, or if
        undo is called from inside event processing before Qt flushes it).
        """
        if not self._ghost_active:
            return

        # Mark inactive FIRST to prevent recursive entry through
        # _on_cursor_moved if cursorPositionChanged fires during deletion.
        self._ghost_active = False
        ghost_text = self._ghost_text
        ghost_start = self._ghost_start
        ghost_end = self._ghost_end
        self._ghost_text = ""
        self._target = None
        self._ghost_start = -1
        self._ghost_end = -1
        ...
        try:
            ...
            # Verify the undo actually removed the ghost text.
            # If it didn't (e.g., another undo step was on top, or undo was
            # a no-op in this execution context), fall back to direct deletion.
            if ghost_start >= 0 and ghost_end > ghost_start:
                plain = doc.toPlainText()
                if (ghost_end <= len(plain)
                        and plain[ghost_start:ghost_end] == ghost_text):
                    logger.debug("doc.undo() failed — cursor deletion fallback")
                    n = doc.characterCount()
                    safe_start = max(0, min(ghost_start, n - 1))
                    safe_end = max(0, min(ghost_end, n - 1))
            ...
            # Restore cursor to the position before ghost text was inserted.
            if ghost_start >= 0:
                n = doc.characterCount()
                safe_pos = max(0, min(ghost_start, n - 1)) if n > 0 else 0
                c = QTextCursor(doc)
    ...
    def _matches_target(self, target):
        ...
        Compares by (line, column) only — not raw offset — because the stored
        offset may be in a different encoding world (\r\n vs \n) than
        cursor.position() which Qt reports in its \n-normalised view.
        """
        if not target:
            return True
        ...
        try:
            cursor = self._editor.textCursor()
            return (
                cursor.blockNumber() == int(target.get("line", -1))
    ...
    def cleanup(self):
        ...
        for target in self._event_targets:
            try:
                ...
                pass
        # Also remove from app in case it was installed there
        try:
            app = QApplication.instance()
            if app is not None:
                app.removeEventFilter(self._event_filter)
