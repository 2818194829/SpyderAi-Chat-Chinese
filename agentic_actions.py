...
def show_confirm_dialog(parent, action_type, target, content, base_dir=None):
    ...
    if action_type in ("file", "patch"):
        ...
        path_lbl = QLabel(f"<b>File:</b> <code>{target}</code>")
        ...
        if base_dir:
            bd_lbl = QLabel(f"📁 Base path: <code>{base_dir}</code>")
        ...
        dir_lbl2 = QLabel(f"<b>Directory:</b> <code>{target or '.'}</code>")
        ...
        _pattern = _gparts[0]
        _scope   = _gparts[1] if len(_gparts) > 1 else "(entire project)"
        grep_lbl = QLabel(
            f"<b>Pattern:</b> <code>{_pattern}</code><br>"
            f"<b>Scope:</b>&nbsp; <code>{_scope}</code>")
...
def show_batch_confirm_dialog(parent, entries, base_dir=""):
    ...
    hdr = QLabel("The following actions were requested by the LLM:")
